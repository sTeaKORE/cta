...
class TestProcessor(
    ...
    override fun process(resolver: Resolver): List<KSAnnotated> {
        ...
        file = codeGenerator.createNewFile(Dependencies(false), "", "tuwien.cta.processor.TestProcessor", "log")
        emit("tuwien.cta.processor.TestProcessor: init($options)", "")
        ...
        emit("tuwien.cta.processor.TestProcessor: process()", "")
        val visitor = TestVisitor()
        for (file in files) {
            emit("tuwien.cta.processor.TestProcessor: processing ${file.fileName}", "")
            file.accept(visitor, "")
        }
        invoked = true
        return emptyList()
    ...
    inner class TestVisitor : KSVisitor<String, Unit> {
        ...
        override fun visitTypeReference(typeReference: KSTypeReference, data: String) {
            ...
            try {
                ...
                emit("tuwien.cta.processor.TestProcessor: exception: $e", data)
            }
        }
